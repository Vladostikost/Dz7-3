...
const container = document.querySelector(".container p-3 mb-2 text-white");
...
const getUSDRate = async (parentTag) => {
    try {
        const response = await axiosInstance.get("/latest", {
            params: {
                base: "USD",
                symbols: "UAH,EUR,GBP,CNY,AUD,BTC",
            },
        });
        const rates = Object.entries(response.date.rates);
...
const getDailyRate = async () => {
    try {
        ...
        const rates = Object.value(response.date.rates).map((rates) =>
